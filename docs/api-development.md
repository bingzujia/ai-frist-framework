...
高级特性用于补充常规 CRUD 场景，覆盖文件上传、参数聚合绑定、JSON 日期格式化和异步任务执行。

### 文件上传

使用 `@RequestPart` + `MultipartFile` 处理 `multipart/form-data` 请求。
...
export class UploadController {
  @PostMapping('/single')
  async uploadSingle(@RequestPart('file') file: MultipartFile): Promise<object> {
    if (file.isEmpty()) {
  ...
  async uploadMulti(
    ...
    const docName = path.basename(document.getOriginalFilename() || 'document.bin');
    const thumbName = path.basename(thumbnail.getOriginalFilename() || 'thumbnail.bin');
...
| 能力 | 说明 | 示例 |
|------|------|------|
| `@RequestPart(name)` | 绑定上传文件字段 | `@RequestPart('file') file: MultipartFile` |
| `MultipartFile` | 统一文件对象接口 | `file.getSize()` |
| `spring.servlet.multipart.maxFileSize` | 控制单文件大小限制 | `5MB` |

### 参数装饰器

`@ModelAttribute` 用于聚合 query/form 参数，`@RequestAttribute` 用于读取中间件注入的请求上下文。
...
export class FormController {
  ...
  search(@ModelAttribute() query: SearchDto): object {
    ...
    return { keyword: query.keyword, page, category: query.category };
  }
  ...
  @GetMapping('/profile')
  profile(@RequestAttribute('currentUser') currentUser: object): object {
    return { currentUser };
  }
}
```
...
| 装饰器 | 用途 | 示例 |
|--------|------|------|
| `@ModelAttribute(name?)` | 合并 query/body 注入对象参数 | `search(@ModelAttribute() query: SearchDto)` |
| `@RequestAttribute(name)` | 读取 `req[name]` 中间件属性 | `profile(@RequestAttribute('currentUser') user: object)` |

### JSON 序列化格式化

`@JsonFormat` 用于控制 DTO 中 `Date` 字段的 JSON 输出格式。

#### 完整示例

```typescript
// src/dto/user-view.dto.ts
...
```typescript
// src/controller/user-view.controller.ts
import {
  RestController,
  GetMapping,
  PathVariable,
...
@RestController({ path: '/user-view' })
export class UserViewController {
  @GetMapping('/:id')
  getById(@PathVariable('id') id: string): UserViewDto {
    const dto = new UserViewDto();
    dto.id = Number(id);
    dto.name = 'Alice';
    dto.createdAt = new Date('2024-01-15T00:30:00Z');
...
| 参数 | 说明 | 示例 |
|------|------|------|
| `pattern` | 日期格式模板 | `yyyy-MM-dd HH:mm:ss` |
| `timezone` | IANA 时区 | `Asia/Shanghai` |
| `shape` | 输出形态（`STRING`/`NUMBER`） | `{ shape: 'NUMBER' }` |

### 异步支持

`@Async` 用于后台执行非阻塞任务，适合通知、日志投递、延迟处理等场景。

#### 完整示例

```typescript
// src/service/notification.service.ts
import { Service, Async } from '@ai-partner-x/aiko-boot';
...
export class NotificationService {
  ...
  async sendWelcomeEmail(userId: number): Promise<void> {
    ...
    console.log('welcome email sent:', userId);
  }
}
```
...
import { RestController, PostMapping, PathVariable } from '@ai-partner-x/aiko-boot-starter-web';
import { Autowired } from '@ai-partner-x/aiko-boot';
import { NotificationService } from '../service/notification.service.js';

@RestController({ path: '/notify' })
export class NotificationController {
  @Autowired()
  private notificationService!: NotificationService;

  @PostMapping('/welcome/:id')
  async welcome(@PathVariable('id') id: string): Promise<object> {
    await this.notificationService.sendWelcomeEmail(Number(id));
    return { accepted: true, userId: Number(id) };
  }
}
```
...
|------|------|
| 返回行为 | 调用方快速返回，后台继续执行 |
| 错误处理 | 使用 `onError` 统一处理后台异常 |
| 适用场景 | 邮件发送、事件投递、异步审计日志 |
