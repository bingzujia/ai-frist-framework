# AI-First Framework — @ai-partner-x/aiko-boot-starter-cache 开发规范
...
| 组件类型 | 文件命名 | 示例 |
|----------|----------|------|
| SPI 接口 | `{name}.ts`（在 `spi/` 目录） | `cache.ts`, `cache-config.ts` |
| 装饰器 | `{name}.ts`（在 `decorators/` 目录） | `decorators.ts` |
| 缓存管理器 | `{backend}-cache-manager.ts` | `redis-cache-manager.ts` |
| 操作接口 | `{type}-operations.ts` | `value-operations.ts` |
| 适配器 | `{driver}-adapter.ts` | `ioredis-adapter.ts` |
| 配置 | `config.ts`, `{scope}-config.ts` | `config.ts`, `cache-config.ts` |
